...
_Formatted from Notes2.txt (content preserved)_
...
> (16) Programming Languages for spark: Python, Scala, R, Java
...
> (40) The "Jobs" tab in a web ui shows the status of all spark jobs in a spark application (i.e. a sparkcontext)
...
> (41) The "Stages" tab is created exclusively when a SparkUI is initialized.

It shows the current state of all stages.

The Stages tab includes: AllStagesPage, StagePage, PoolPage

> (42) The "Storage" tab displays the information about RDDs
...
> (43) The "Environment" tab displays the values of different environment and configuration variables

It shows Java, Spark, and system properties

The Environment tab has: Parent SparkUI, AppStatusStore

> (44) The "Executors" tab displays the summary information about the executors that were created for the application
...
> (51) RDDs are the main logical data unit in spark.
...
> (58) # Uploading a file to Google Colab:
...
> (63) Transformations are lazy operations on an RDD that create one or more new RDDs.
...
> (64) Given below is a list of RDD Transformations:
...
> (74) rdd = sc.parallelize([1,2,3,4],4) 
...
> (78) zip joins two RDDs by combining the i^{th} part of either partition with each other.
...
> (81) Difference Between Coalesce() and Repartition():
...
> (92) max,min,sum,mean,variance and stdev:
...
> (96) Creating Paired RDDs (working with key-value pair):
...
> (97) rdd = sc.parallelize([("a1","b1","c1","d1","e1"),("a2","b2","c2","d2","e2")])
result = rdd.map(lambda x: (x[0],list(x[1:])))
result.collect()

o/p: [('a1',['b1','c1','d1','e1']),('a2',['b2','c2','d2','e2'])]

> (98) Transformations on Paired RDDs:
...
> (100) WordCount using RDD concepts:
