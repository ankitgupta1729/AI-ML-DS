...
def camera(self):
    ...
    if SaveVideo:
        # Upstream hard-codes ./video.avi with XVID. --saveVideoPath lets the
        # output land anywhere, and the extension selects the codec so that
        # `.mp4` produces a file QuickTime can play on macOS.
        out_path = self.FLAGS.saveVideoPath or 'video.avi'
        out_dir = os.path.dirname(out_path)
        if out_dir and not os.path.exists(out_dir):
            os.makedirs(out_dir)
        codec = 'mp4v' if os.path.splitext(out_path)[1].lower() in ('.mp4', '.m4v', '.mov') else 'XVID'
        ...
        'Cannot open {} for writing with codec {}'.format(out_path, codec)
        self.say('Writing {} ({} @ {} fps)'.format(out_path, codec, fps))
