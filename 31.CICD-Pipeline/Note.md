...
[Make sure Add that SP to the workspace as an admin.
In your Databricks workspace (adb-7405605083397751.11.azuredatabricks.net):

Top-right (your avatar) → Settings → Identity and access.

Click Add service principal again.
On the first screen of the dialog, do NOT pick "Add new." Look for the option to add an existing service principal (a search box for existing account principals). If you're already on the "Management / Microsoft Entra ID managed" form, click Back to get to that first screen.
Search by the Application ID 4c02c10c-9988-4595-918c-dd8d9c60f0e1 (or by name). Select the existing principal that appears.
Turn Admin access → On, then Add. This assigns the already-existing account SP to the workspace as an admin (no new account SP is created, so no collision).
]

12. Now, we focus on CD(Continuous Deployment/Delivery) part. We deploy the changes from main branch of dev environment to live folder of prod environment.

First, in vscode, open the cicd-pipelines.yml file with main branch and create a new branch called `feature/cd-pipeline` from main branch.

Here, we have to add prod related group and stages in cicd-pipelines.yml file.

Now, we have to add variable group,environment and service connection for prod in azure devops.

Add a new variable group as ` dbw-cicd-prod` and add variables as

- `prod-environment-name` with value as `prod-environment-databricks-cicd`
- `prod-resource-group-name` with value as `azure-data-engineering-project-prod`
- `prod-service-connection-name` with value as `prod-service-connection`

Now, create the new environment with name as `prod-environment-databricks-cicd` in pipelines tab on azure devops for prod.

Now, create a new service connection with name as `prod-service-connection` in pipelines tab on azure devops for prod.

AAt the end give pipeline permissions for resource group,environments and service connection as we did for dev.

Now, commit all the changes in `feature/cd-pipeline` branch in vscode with commit message as `added cd pipeline`.

Now, create a new pull request from `feature/cd-pipeline` branch to `main` branch in azure devops portal.

After that pipeline will automatically triggered and we can see in the pipelines tab and build will be having 2 stages.

13. On Azure devops portal, there is an option of `Approval and checks` and it is used between dev and prod pipeline. When dev got succedeed and then we need some high level approval like tech lead and then we need to deploy the changes to prod environment. 

For this, on azure devops portal, go to prod environment in pipelines tab and then go to `Approval and checks` and select `Approval`  from + symbol. Then add the mail id of a person from which you need approval and then a mail will be triggered to that person and in this way prod environment will be protected.

Now, for full cicd testing,

go to dev workspace and add a new branch called `end-to-end-cicd-testing` from main branch. Now, create a new notebook `final-cicd-test-notebook` and commit and push the changes.

Now, create a new pull request from `end-to-end-cicd-testing` branch to `main` branch in azure devops portal.
