...
def generate_response(retriever, query):
    ...
    | model 
    | StrOutputParser() # here we are creating a chain that takes the user query as input, passes it through the prompt template and then generates a response using the model and finally parses the output to a string
    )
    return chain.invoke(query) # here we are invoking the chain with the user query as input

def query(query):
    """Query the model with a user query."""
    documents = load_split_documents()
    retriever = load_embeddings(documents, query)
    return generate_response(retriever,query) # here we are generating a response to the user query using the generate_response function
