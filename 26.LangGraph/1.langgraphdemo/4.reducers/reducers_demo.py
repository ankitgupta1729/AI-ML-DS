...
def sales_response(state: ChatBotState):
    return {
        "messages": [AIMessage(content="We have the best offer for you!")], 
        "discount": 20
    }
...
messages = chatbot.invoke({
    "messages": [HumanMessage(content=test_inputs)]
})

for message in messages["messages"]:
    print(f"**Bot**: {message.content}")
    
print(f"Final Discount: ", messages["discount"],'%')