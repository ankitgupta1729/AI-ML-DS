...
def post_image(file):
    ...
    """
    #using Pillow -- python image processing -- to turn the posted file into bytes
    ...
    #scale to 0..1 exactly as train_mnist.py scaled the training data
    image_array = image_array.astype('float32') / 255.0
    prediction = MNIST_MODEL.predict(image_array)
    #argmax to reverse the one hot encoding
    digit = np.argmax(prediction[0])
    #need to convert to int/float -- numpy scalars aren't known to serialize
    return json.dumps({
        'digit': int(digit),
